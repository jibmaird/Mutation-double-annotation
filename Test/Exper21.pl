...
my $NER_DIR = "/aitu-stor/dmiraola/Experiments/Mutation/21";
...
foreach my $t (@T) {
    ...
    if (not -e "$NER_DIR/bert_out/$t") {
	system("mkdir $NER_DIR/bert_out/$t");
    }   
...
#eval entity-level
    ...
    system("perl ./biocodes/conlleval.pl < $NER_DIR/bert_out/$t/NER_result_conll.txt > $NER_DIR/bert_out/$t/eval_entity.txt");
}
