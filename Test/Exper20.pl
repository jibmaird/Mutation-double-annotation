...
for(my $i=1;$i<=$reps;$i++) {
	...
	system("python run_ner.py --do_train=true --do_eval=true --vocab_file=$BIOBERT_DIR\/vocab.txt --bert_config_file=$BIOBERT_DIR\/bert_config.json --init_checkpoint=$BIOBERT_DIR\/biobert_model.ckpt --num_train_epochs=10.0 --data_dir=$NER_DIR\/bio\/$t\/$i\/ --output_dir=$NER_DIR/bert_out/$t\_$i");
#eval entity-level
	system("python biocodes/ner_detokenize.py --token_test_path=$NER_DIR/bert_out/$t\_$i/token_test.txt --label_test_path=$NER_DIR/bert_out/$t\_$i/label_test.txt --answer_path=$NER_DIR/bio/$t\/$i/test.tsv --output_dir=$NER_DIR/bert_out/$t\_$i");
...
chdir("/home/dmiraola/aitu-ner-data-consistency/Test");
